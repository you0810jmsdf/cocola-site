...
Google Apps Script (GAS) を Web App として公開し、登録は GAS 直接、閲覧は GitHub Pages 上の
カレンダー UI (`calendar/`) が `schedule/data.json` (Actions ミラー) を読む構成。

```
[登録] form.html (GAS hosted)
        │  google.script.run
        ▼
[GAS Web App: cocola_calendar.gs] ── createEvent_ ──▶ [Google カレンダー]
                  │
                  │ ?mode=list (5分ごと)
                  ▼
[GitHub Actions: update-schedule-data.yml]
                  │  curl → JSON 整形 → commit
                  ▼
[GitHub Pages] schedule/data.json ◀── fetch ── [calendar/index.html]
```

GitHub Pages から GAS を直接叩かないのは、CORS / レート / 速度のため。
TOPICS / DAO / Members と同じ「GAS → Actions → data.json → 静的ページ」パターンを踏襲する。
...
├── form.html               ← 予定登録フォーム (GAS hosted)
├── schedule.html           ← 予定一覧 (GAS hosted、軽量フォールバック)
├── data.json               ← Pages 公開用ミラー (Actions が自動更新、手で編集しない)
└── セットアップ手順.txt    ← clasp / Web App デプロイ手順

calendar/
└── index.html              ← Pages 上のカレンダー UI。schedule/data.json を読む
```
...
- [ ] `?mode=list` で `{ ok:true, events:[...] }` が返る
- [ ] `form.html` から予定登録 → Google カレンダーに反映される
- [ ] 合言葉なし / 誤りで POST → `{ ok:false }` が返る
- [ ] リポジトリ Variables に `GAS_SCHEDULE_URL` を設定し `update-schedule-data.yml` を手動実行 → `schedule/data.json` が更新される
- [ ] `calendar/` を Pages 上で開く → 月グリッドに登録予定が点で表示される
- [ ] 日付タップで予定詳細が表示される
